...
def parse_args() -> argparse.Namespace:
    ...
    # Logging is OFF by default: writing into a user's home directory is not
    # something a scaffolding tool should do unasked. --log opts in.
    parser.add_argument(
        "--log",
        action="store_true",
        help="Record a usage log (off by default; see --log-dir for the location)",
    )
    parser.add_argument(
        "--log-dir",
        default=None,
        help="Where to write the usage log. Implies --log.",
    )
    parser.add_argument(
        "--no-log",
        action="store_true",
        help="Deprecated: logging is already off by default. Kept so existing "
             "invocations keep working.",
    )
    parser.add_argument("--dry-run", action="store_true", help="Print file plan without writing anything")
    parser.add_argument(
        "--yes",
        "-y",
        action="store_true",
...
def choose_action(dest: Path, overwrite_mode: str) -> str:
    ...
    # ask-each needs a human. Called by an agent, input() raises EOFError on the
    # first existing file, which used to surface as a traceback mid-write.
    # Refuse up front and name the two ways to proceed.
    if not stdin_is_interactive():
        raise SystemExit(
            f"Refusing to continue: '{dest}' already exists and the overwrite "
            f"strategy is 'ask-each', which needs an interactive terminal.\n"
            f"Re-run with an explicit policy instead:\n"
            f"  --overwrite skip-existing    keep every existing file\n"
            f"  --overwrite overwrite-all    replace them\n"
            f"Or preview first with --dry-run."
        )

    while True:
        try:
            answer = (
                input(f"File exists: {dest}\nChoose [o]verwrite / [s]kip / [q]uit: ")
                .strip()
                .lstrip("\ufeff")
                .lower()
            )
            ...
            raise SystemExit(
                "stdin closed while waiting for an overwrite decision; nothing further "
                "was written. Re-run with --overwrite skip-existing or overwrite-all."
            ) from None
...
def resolve_log_dir(args: argparse.Namespace) -> Path | None:
    """Where to write the usage log, or None to write none.

    Off unless explicitly requested. A dry run never logs — it promises to write
    nothing, and a log file is a write.
    """
    if args.no_log or args.dry_run:
        return None
    if not (args.log or args.log_dir):
        return None
...
def main() -> int:
    ...
    try:
        ...
        # Decide before writing anything. Checking per-file meant the first few
        # targets were already on disk by the time an existing later one forced a
        # stop — a half-scaffolded package is worse than none, and the user has
        # to work out which files are theirs.
        if not args.dry_run and overwrite_mode == "ask-each" and not stdin_is_interactive():
            existing = [dest for _, dest in plan if dest.exists()]
            if existing:
                listed = "\n".join(f"  {p}" for p in existing)
                raise SystemExit(
                    f"Refusing to start: {len(existing)} target file(s) already exist and the "
                    f"overwrite strategy is 'ask-each', which needs an interactive terminal.\n"
                    f"{listed}\n"
                    f"Nothing was written. Re-run with an explicit policy:\n"
                    f"  --overwrite skip-existing    keep every existing file\n"
                    f"  --overwrite overwrite-all    replace them"
                )
        ...
        if args.yes:
            ...
            # No human to ask, and generating files unasked is worse than
            # stopping. --yes is the explicit opt-in.
            raise SystemExit(
                "Refusing to write files without confirmation: stdin is not a terminal.\n"
                "Pass --yes to confirm non-interactively, or --dry-run to see the plan."
            )
        else:
            try:
                ...
                raise SystemExit(
                    "stdin closed before confirmation; no files were written. "
