...
class OperatorDetailSchema(BaseModel):
    ...
    mode: str
    agent_tips: Optional[List[str]] = []
    field_binding_hint: Optional[str] = None
    mcp_context_hint: Optional[str] = None


class OperatorCategoryRecommendationItem(BaseModel):
    category: str
    score: int
    reason: str


class OperatorCategoryRecommendationRequest(BaseModel):
    task_description: str = Field(..., min_length=1)
    dataset_columns: Optional[List[str]] = []
    max_categories: int = Field(default=2, ge=1, le=3)


class OperatorCategoryRecommendationSchema(BaseModel):
    recommended_categories: List[OperatorCategoryRecommendationItem]
    avoid_categories: Optional[List[OperatorCategoryRecommendationItem]] = []
    next_action: str
